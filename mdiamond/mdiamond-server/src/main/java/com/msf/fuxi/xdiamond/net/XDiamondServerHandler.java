...
import com.msf.fuxi.xdiamond.common.net.*;
import com.msf.fuxi.xdiamond.common.net.Message.MessageBuilder;
import com.msf.fuxi.xdiamond.common.net.Response.ResponseBuilder;
...
//          String errorMsg = String.format("SecretKey is wrong! The secretKey of config is [%s]", secretKey);
          String errorMsg = String.format("SecretKey is wrong! The secretKey of project is [%s], and the secretKey of config is [%s]",
                  projectProfile.getSecretKey(), secretKey);
          Response response =
              responseBuilder.fail().withErrorMessage(errorMsg)
                  .command(request.getCommand()).build();
          messageBuilder.jsonData(response);
          ctx.writeAndFlush(messageBuilder.build());
          // 设置connection错误信息
          connectionInfo.setMessage(errorMsg);
          return;
        }
